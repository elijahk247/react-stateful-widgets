...
export default function Programmers() {
  ...
  const [activeAwesome, setActiveAwesome] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    console.log(activeAwesome)

    console.log(typeof(activeAwesome))
    console.log(Number(activeAwesome));
    console.log(awesome[Number(activeAwesome) - 1].name);
    console.log(awesome)

    const index = Number(activeAwesome) - 1;
    console.log(index)
    
    //return awesome.find(p => p.id === activeAwesome)?.name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeAwesome === null ? 'royalblue' : 'yellow', // 🤔
  };
      ...
      <div className='programmers'>
        {
          ...
          awesome.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => { /* set the featured id passing dev.id */
              setActiveAwesome(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
...
//     fontSize: '1.5em',
//     marginTop: '0.5em',
//     color: featured ? 'gold' : 'royalblue', // :thinking_face:
//   };
//   return (
//     <div className='widget-programmers container'>
//       <h2>Programmers</h2>
...
//         // Pseudo-code: if the currently featured id is truthy render div 1, otherwise render div 2. Fix!
//         featured
//           ? <div style={style}>:tada: Let&apos;s celebrate {getNameOfFeatured()}! 🥳</div>
//           : <div style={style}>Pick an awesome programmer</div>
//       }
//     </div>
//   );
